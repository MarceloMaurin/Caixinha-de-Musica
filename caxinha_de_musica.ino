...
int pinledOutButton = 4;  // Analog input pin that the potentiometer is attached to
int pinledInButton = 5; // Analog output pin that the LED is attached to
...
void Mensagem(){
  ...
  sprintf(info,"Temperatura:%i C",temp);
  Serial.println(info);
  lcd.print(info);
  lcd.setCursor(2,3);
  sprintf(info,"%s",lastmusica);
  lcd.print(info);
  delay(300);
}
